# SenyaMatika - Mathematics Learning App

Project Overview

<<<<<<< HEAD
SenyaMatika is an innovative educational Flutter application designed to make learning mathematics fun and engaging. The app provides interactive lessons, quizzes, progress tracking, and sign language support to help students master mathematical concepts.
=======
SenyaMatika is an innovative educational Flutter application designed to make learning mathematics fun and engaging. The app provides interactive lessons, progress tracking, and sign language support to help students master mathematical concepts.
>>>>>>> 79d8d86d2e070a44957bc6d05e52b20b9b440f53

# Features

Core Features

<<<<<<< HEAD
- Interactive Learning Space: Access mathematics topics with visual content

- Sign Language Avatar: Real-time speech-to-sign language translation

- Progress Tracking: Monitor learning progress with detailed analytics

- Quiz System: Test knowledge with various quiz formats

- Sign Language Dictionary: Learn sign language for mathematical terms

# Topics Covered
- Number Values (Whole Numbers, Comparison, Ordinal Numbers, Money Value)

- Fundamental Operations (Addition, Subtraction, Multiplication, Division)

- Fraction (Basic concepts)

- Decimal Numbers (Basic concepts)

- Percentage (Concepts of percentage)

- Mensuration (Time, Days/Weeks/Months)

# SET Up Guide

Prerequisites
- Flutter SDK: Version 3.9.2 or higher

- Dart SDK: Version 3.x

- IDE: Android Studio, VS Code, or any Flutter-compatible IDE

- Device: Android emulator, or physical device

=======
- Interactive Learning Space: Access mathematics topics with visual content and assessment

- Sign Language Avatar: Text-to-sign language translation

- Progress Tracking: Monitor learning progress with detailed analytics

- Sign Language Dictionary: Learn sign language for mathematical terms

# Topics Covered
- Number Values (Whole Numbers, Comparison, Ordinal Numbers, Money Value)

- Fundamental Operations (Addition, Subtraction, Multiplication, Division)

- Fraction (Basic concepts)

- Decimal Numbers (Basic concepts)

- Percentage (Concepts of percentage)

- Mensuration (Time, Days/Weeks/Months)

# SET Up Guide

Prerequisites
- Flutter SDK: Version 3.9.2 or higher

- Dart SDK: Version 3.x

- IDE: Android Studio, VS Code, or any Flutter-compatible IDE

- Device: Android emulator, or physical device

# Dependencies & Libraries
The app uses the following packages (already listed in pubspec.yaml):

# Core Dependencies
flutter: UI framework

video_player (^2.8.2): For playing educational videos

speech_to_text (^7.3.0): Speech recognition for avatar translator

permission_handler (^11.0.1): Manage app permissions

shared_preferences (^2.2.2): Local storage for user preferences

google_fonts (^5.0.0): Custom typography

cupertino_icons (^1.0.8): iOS-style icons

Development Dependencies
flutter_test: Testing framework

flutter_lints (^5.0.0): Code quality and linting

flutter_launcher_icons (^0.14.4): Generate app icons
>>>>>>> 79d8d86d2e070a44957bc6d05e52b20b9b440f53
